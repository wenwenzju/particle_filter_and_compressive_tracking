...
private:
	...
	void getFeatureValue(vector<Mat>& _imageIntegral, Rect& _sampleBox, Mat& _sampleFeatureValue);
	void classifierUpdate(Mat& _sampleFeatureValue, vector<float>& _mu, vector<float>& _sigma, float _learnRate);
	void classifierUpdate(Mat& posFeatureValue, Mat& negFeatureValue);
	void radioClassifier(vector<float>& _muPos, vector<float>& _sigmaPos, vector<float>& _muNeg, vector<float>& _sigmaNeg,
						Mat& _sampleFeatureValue, float& _radioMax, int& _radioMaxIndex);
	void radioClassifier(Mat& sampleFeatureValue, float& radioMax, int& radioMaxIndex);
	Rect radioClassifier(Mat& sampleFeatureValue, vector<Rect>& rects);
